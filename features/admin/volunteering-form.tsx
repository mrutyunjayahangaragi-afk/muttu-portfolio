...
type FormValues = z.infer<typeof volunteeringSchema>
...
export function VolunteeringForm({ volunteering, onSubmit }: VolunteeringFormProps) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()
  const [logoUrl, setLogoUrl] = useState(volunteering?.logo_url ?? "")
  ...
  const {
    ...
    defaultValues: {
      ...
      current: volunteering?.current ?? false,
      description: volunteering?.description ?? "",
      impact: volunteering?.impact ?? "",
      display_order: volunteering?.display_order ?? 0,
    },
  })
  ...
  async function submit(values: FormValues) {
    ...
    fd.append("impact", values.impact || "")
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        setSaved(true)
        setTimeout(() => router.push("/admin/volunteering"), 800)
      } else {
        setServerError(result.error ?? "Failed to save volunteering entry")
      }
    })
  }
  ...
  return (
    <form onSubmit={handleSubmit(submit)} noValidate className="space-y-6 max-w-2xl">
      <div className="glass rounded-2xl p-6 border border-white/10 space-y-4">
        <h3 className="text-sm font-semibold text-white">Volunteering Activity</h3>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-white/60 mb-1.5 block">Role / Title *</label>
            <Input {...register("title")} placeholder="e.g. Volunteer Mentor / Code Instructor" />
            {errors.title && <p className="text-xs text-red-400 mt-1">{errors.title.message}</p>}
          </div>

          <div>
            <label className="text-xs text-white/60 mb-1.5 block">Organization *</label>
            <Input {...register("organization")} placeholder="e.g. CoderDojo / Red Cross" />
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          <span className="text-sm text-white/70">Currently volunteering here</span>
        </label>

        <div>
          <label className="text-xs text-white/60 mb-1.5 block">Description</label>
          <Textarea {...register("description")} rows={3} placeholder="Details of volunteer work..." />
        </div>

        <div>
          <label className="text-xs text-white/60 mb-1.5 block">Community Impact</label>
          <Input {...register("impact")} placeholder="e.g. Taught coding to 200+ students" />
        </div>
      ...
      <div className="flex items-center gap-3">
        <Button type="submit" variant="gradient" disabled={isPending} className="gap-2">
          ...
          {volunteering ? "Save Changes" : "Create Volunteering Entry"}
        </Button>
        <Button type="button" variant="outline" onClick={() => router.push("/admin/volunteering")}>
          Cancel
        </Button>
      </div>
    </form>
