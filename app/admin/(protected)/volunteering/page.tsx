...
import { deleteVolunteering } from "../actions"

export const metadata: Metadata = {
  title: "Volunteering — Admin",
...
export default async function AdminVolunteeringPage() {
  ...
  return (
    <div>
      <AdminPageHeader title="Volunteering" description={`${items?.length ?? 0} items`}>
        <a
          href="/admin/volunteering/new"
      ...
      <div className="space-y-3">
        ...
        {items?.map((item) => (
          ...
          >
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-pink-500/20 to-rose-500/20 text-lg">
              🤝
            </div>
            ...
            <div className="flex shrink-0 items-center gap-2">
              <a
                href={`/admin/volunteering/${item.id}/edit`}
                className="rounded-lg p-2 text-white/50 transition-colors hover:bg-blue-500/10 hover:text-blue-400"
              >
                <Pencil size={14} />
              </a>
              <DeleteButton id={item.id} action={deleteVolunteering} label="volunteering entry" />
            </div>
          </div>
        ))}
      </div>
